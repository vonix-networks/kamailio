...
void cfg_pos_join(
		struct cfg_pos *res, struct cfg_pos *pos1, struct cfg_pos *pos2)
{
	struct cfg_pos ret;
	ret = *pos1;
...
struct expr *mk_elem(int op, expr_l_type ltype, void *lparam, expr_r_type rtype,
		void *rparam)
{
	struct expr *e;
	e = (struct expr *)pkg_malloc(sizeof(struct expr));
...
struct action *mk_action(enum action_type type, int count, ...)
{
	...
	for(i = 0; i < a->count; i++) {
		...
		DBG("ACTION type%d, i/count %d/%d: type %d(%x)/ data %p\n", a->type, i,
				a->count, a->val[i].type, a->val[i].type, a->val[i].u.data);
	}
	va_end(args);
	...
	switch(t->val[1].type) {
		...
		case AVP_ST:
			DBG(", avp(%u,%.*s)", t->val[1].u.attr->type,
					t->val[1].u.attr->name.s.len,
					ZSW(t->val[1].u.attr->name.s.s));
			break;
		case SELECT_ST:
			DBG("select");
			break;

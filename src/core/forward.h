...
struct socket_info *get_send_socket2(struct socket_info *force_send_socket,
		union sockaddr_union *su, int proto, enum ss_mismatch *mismatch);
...
#define GET_URI_PORT(uri)            \
	((uri)->port_no ? (uri)->port_no \
					: (((uri)->proto == PROTO_TLS) ? SIPS_PORT : SIP_PORT))

struct socket_info *get_out_socket(union sockaddr_union *to, int proto);
typedef int (*check_self_f)(
...
#define update_sock_struct_from_ip(to, msg)                   \
	init_su((to), &(msg)->rcv.src_ip,                         \
			(((msg)->via1->rport)                             \
					|| (((msg)->msg_flags | global_req_flags) \
							& FL_FORCE_RPORT))                \
					? (msg)->rcv.src_port                     \
			: ((msg)->via1->port) ? (msg)->via1->port         \
								  : SIP_PORT)
...
static inline int msg_send_buffer(
		struct dest_info *dst, char *buf, int len, int flags)
{

/* Reconstructed (2026-09): C++-includable copy of str_hash.h for the kazoo
 * module (kz_jwt.h includes it inside extern "C"). */
/*
...
inline static void str_hash_init(struct str_hash_table *ht)
{
	...
	for(r = 0; r < ht->size; r++)
		ht->table[r].next = ht->table[r].prev = (struct str_hash_entry *)&ht->table[r];
}
		...
		struct str_hash_table *ht, struct str_hash_entry *e)
{
	...
	e->next = ht->table[h].next;
	e->prev = (struct str_hash_entry *)&ht->table[h];
	ht->table[h].next->prev = e;
	ht->table[h].next = e;
}
		...
		struct str_hash_table *ht, const char *key, int len)
{
	...
	for(e = (struct str_hash_entry *)(&ht->table[h])->next;
			e != (struct str_hash_entry *)(&ht->table[h]); e = e->next) {

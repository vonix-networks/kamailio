...
#define IFISMETHOD(methodname, firstchar)                                  \
	if((*tmp == (firstchar) || *tmp == ((firstchar) | 32))                 \
			&& strncasecmp(tmp + 1, &#methodname[1], methodname##_LEN - 1) \
					   == 0                                                \
			&& *(tmp + methodname##_LEN) == ' ') {                         \
		fl->type = SIP_REQUEST;                                            \
...
/* sip message */
#define IS_SIP_MSG(req) ((req)->first_line.flags & FLINE_FLAG_PROTO_SIP)
...
typedef struct sip_msg
{
	...
	int parsed_orig_ruri_ok; /*!< 1 if parsed_orig_uri is valid, 0 if not, set if to 0
								if you modify the uri (e.g change new_uri)*/
	struct sip_uri
			parsed_orig_ruri; /*!< speed-up > keep here the parsed orig uri*/

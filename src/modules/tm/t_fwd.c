 ...
 */
static int prepare_new_uac(struct cell *t, struct sip_msg *i_req, int branch,
		str *uri, str *path, str *next_hop, struct socket_info *fsocket,
		snd_flags_t snd_flags, int fproto, int flags, str *instance, str *ruid,
		str *location_ua)
{
	char *shbuf;
		...
		char *buf, short buf_len)
{
	...
	*len = buf_len + via_len - (old_via_end - old_via_begin);
	...
	memcpy(shbuf + (old_via_begin - buf) + via_len, old_via_end,
			(buf + buf_len) - old_via_end);
		...
		str *location_ua)
{
	...
	if(unlikely((ret = prepare_new_uac(t, request, branch, uri, path, next_hop,
						 fsocket, snd_flags, proto, flags, instance, ruid,
						 location_ua))
				< 0)) {
		ser_error = ret;
		goto error01;
 ...
 */
static int add_uac_from_buf(struct cell *t, struct sip_msg *request, str *uri,
		str *path, struct socket_info *fsocket, snd_flags_t send_flags,
		int proto, char *buf, short buf_len, str *instance, str *ruid,
		str *location_ua)
{
	...
	/* now message printing starts ... */
	shbuf = print_uac_request_from_buf(t, request, branch, uri, &len,
			&t->uac[branch].request.dst, buf, buf_len);
	if(!shbuf || len <= 0) {
		if(shbuf) {
			shm_free(shbuf);
			shbuf = NULL;
		...
		struct ua_client *old_uac, int lock_replies)
{
	...
	if(cfg_get(core, core_cfg, use_dns_failover)
		...
		t->uac[t->nr_of_outgoings].on_branch_failure =
		...
		if(cfg_get(tm, tm_cfg, reparse_on_dns_failover)) {
			...
			ret = add_uac_from_buf(t, msg, &old_uac->uri, &old_uac->path,
					(old_uac->request.dst.send_flags.f & SND_F_FORCE_SOCKET)
							? old_uac->request.dst.send_sock
							: ((tm_dns_reuse_rcv_socket) ? msg->rcv.bind_address
														 : 0),
					old_uac->request.dst.send_flags, old_uac->request.dst.proto,
					old_uac->request.buffer, old_uac->request.buffer_len,
					&old_uac->instance, &old_uac->ruid, &old_uac->location_ua);
		} else {
			...
			ret = add_uac(t, msg, &old_uac->uri, 0, &old_uac->path, 0,
					(old_uac->request.dst.send_flags.f & SND_F_FORCE_SOCKET)
							...
							: ((tm_dns_reuse_rcv_socket) ? msg->rcv.bind_address
														 : 0),
					old_uac->request.dst.send_flags, old_uac->request.dst.proto,
					UAC_DNS_FAILOVER_F, &old_uac->instance, &old_uac->ruid,
					&old_uac->location_ua);
		}
		...
		struct cell *t_invite, int branch)
{
	...
	if(cfg_get(tm, tm_cfg, reparse_invite)) {
		...
		t_cancel->uac[branch].uri.s =
				t_cancel->uac[branch].request.buffer
		...
		if(unlikely((ret = prepare_new_uac(t_cancel, cancel_msg, branch,
							 ...
							 &t_invite->uac[branch].path, 0, 0, snd_flags,
							 PROTO_NONE, 0, NULL, NULL, NULL))
					< 0)) {
			ser_error = ret;
			goto error;
		}
 ...
 *        0 on error
 */
static struct cancel_reason *cancel_reason_pack(
		short cause, void *data, struct cell *t)
{
	char *d;
	...
	if(likely(cause != CANCEL_REAS_UNKNOWN)) {
		...
		reas1 = 0;
		reas_last = 0;
		if(likely(cause == CANCEL_REAS_RCVD_CANCEL && data
				   && !(t->flags & T_NO_E2E_CANCEL_REASON))) {
			...
			for(hdr = get_hdr(e2e_cancel, HDR_REASON_T), reas1 = hdr; hdr;
			...
			reason_len =
					REASON_PREFIX_LEN + USHORT2SBUF_MAX_LEN
		...
		if(cause == CANCEL_REAS_RCVD_CANCEL) {
			...
			code_len = ushort2sbuf(
					cause, d, reason_len - (int)(d - (char *)cr - sizeof(*cr)));
		...
		struct cell *t_invite)
{
	...
	if(unlikely(has_tran_tmcbs(t_invite, TMCB_E2ECANCEL_IN))) {
		...
		run_trans_callbacks_internal(
				&t_invite->tmcb_hl, TMCB_E2ECANCEL_IN, t_invite, &tmcb);
	}
...
#ifdef USE_DST_BLOCKLIST
	if(cfg_get(core, core_cfg, use_dst_blocklist) && p_msg
			&& (p_msg->REQ_METHOD
	...
	if(SEND_BUFFER(&uac->request) == -1) {
		...
		if(unlikely(has_tran_tmcbs(t, TMCB_REQUEST_SENT)))
			run_trans_callbacks_with_buf(
					TMCB_REQUEST_SENT, &uac->request, p_msg, 0, TMCB_NONE_F);
...
int t_forward_nonack(
		struct cell *t, struct sip_msg *p_msg, struct proxy_l *proxy, int proto)
{
	...
	while(next_branch_data(&obranch) == 1) {
		...
		branch_ret = add_uac(t, p_msg, &obranch.uri,
				...
				&obranch.path, proxy, obranch.force_socket,
				p_msg->fwd_send_flags, proto,
				(obranch.dst_uri.len > 0) ? 0 : UAC_SKIP_BR_DST_F,
				...
				if(branch_ret == i) { /* success */
					...
					if(unlikely(has_tran_tmcbs(t, TMCB_REQUEST_OUT)))
						run_trans_callbacks_with_buf(TMCB_REQUEST_OUT,
								&t->uac[i].request, p_msg, 0, TMCB_NONE_F);

...
int pv_xavp_get_value(
		struct sip_msg *msg, pv_param_t *param, pv_value_t *res, sr_xavp_t *avp)
{
 ...
 */
int pv_set_xavp(struct sip_msg *msg, pv_param_t *param, int op, pv_value_t *val)
{
	pv_xavp_name_t *xname = NULL;
	sr_xavp_t *avp = NULL;
	sr_xavp_t *list = NULL;
	...
	if((val == NULL) || (val->flags & PV_VAL_NULL)) {
		...
		if(xname->next->index.type == PVT_EXTRA) {
			/* get the index */
			if(pv_get_spec_index(msg, &xname->next->index.pvp, &idx1, &idxf1)
					!= 0) {
				LM_ERR("invalid index!\n");
				return -1;
			}
			...
			while(avp) {
				if(avp->val.type == SR_XTYPE_XAVP) {
					if(xname->next->index.type == PVT_EXTRA) {
						if(idxf1 == PV_IDX_ALL) {
							xavp_rm_by_name(
									&xname->next->name, 1, &avp->val.v.xavp);
						} else {
							/* fix the index */
							idx = idx1;
							if(idx < 0) {
								count = xavp_count(
										&xname->next->name, &avp->val.v.xavp);
								idx = count + idx1 + 1;
							}
							xavp_rm_by_index(
									&xname->next->name, idx, &avp->val.v.xavp);
						}
					} else {
						xavp_rm_by_name(
								&xname->next->name, 0, &avp->val.v.xavp);
					}
				}
		...
		if(avp) {
			if(avp->val.type == SR_XTYPE_XAVP) {
				if(xname->next->index.type == PVT_EXTRA) {
					...
					xavp_rm_by_name(&xname->next->name, 0, &avp->val.v.xavp);
				}
			}
		}
		return 0;
...
int xavp_slist_explode(str *slist, str *sep, str *mode, str *xname)
{
	...
	if(slist == NULL || xname == NULL || slist->s == NULL || xname->s == NULL
			|| slist->len <= 0 || xname->len <= 0 || sep == NULL
			|| sep->s == NULL || sep->len <= 0 || mode == NULL) {
...
int pv_var_to_xavp(str *varname, str *xname)
{
	...
	if(varname->len == 1 && varname->s[0] == '*') {
		...
		if(it->v.flags == VAR_VAL_INT) {
			...
			xval.v.s.s = it->v.value.s.s;
			xval.v.s.len = it->v.value.s.len;
			LM_DBG("[%.*s]: '%.*s'\n", it->name.len, it->name.s, xval.v.s.len,
					xval.v.s.s);
		}
...
int pv_xavp_to_var_helper(sr_xavp_t *avp)
{
	...
	if(avp->val.type == SR_XTYPE_STR) {
		...
		flags |= VAR_VAL_INT;
		value.n = avp->val.v.l;
		LM_DBG("var:[%.*s] INT:[%ld]\n", avp->name.len, avp->name.s, value.n);
	} else {
...
int pv_xavp_to_var(str *xname)
{
	...
	do {
		avp = xavp->val.v.xavp;
		if(avp) {
			if(pv_xavp_to_var_helper(avp) < 0)
				return -1;
			avp = avp->next;
		}
 ...
 * $xavu(name1=>name2)
 */
int pv_set_xavu(struct sip_msg *msg, pv_param_t *param, int op, pv_value_t *val)
{
	pv_xavu_name_t *xname = NULL;
	sr_xavp_t *avu = NULL;
	sr_xval_t xval;
 ...
 */
int pv_set_xavi(struct sip_msg *msg, pv_param_t *param, int op, pv_value_t *val)
{
	pv_xavp_name_t *xname = NULL;
	sr_xavp_t *avi = NULL;
	sr_xavp_t *list = NULL;
			...
			while(avi) {
				if(avi->val.type == SR_XTYPE_XAVP) {
					if(xname->next->index.type == PVT_EXTRA) {
						if(idxf1 == PV_IDX_ALL) {
							xavi_rm_by_name(
									&xname->next->name, 1, &avi->val.v.xavp);
						} else {
							/* fix the index */
							idx = idx1;
							if(idx < 0) {
								count = xavi_count(
										&xname->next->name, &avi->val.v.xavp);
								idx = count + idx1 + 1;
							}
							xavi_rm_by_index(
									&xname->next->name, idx, &avi->val.v.xavp);
						}
					} else {
						xavi_rm_by_name(
								&xname->next->name, 0, &avi->val.v.xavp);
					}
				}
		...
		if(avi) {
			if(avi->val.type == SR_XTYPE_XAVP) {
				if(xname->next->index.type == PVT_EXTRA) {
					...
					xavi_rm_by_name(&xname->next->name, 0, &avi->val.v.xavp);
				}
			}
		}
		return 0;

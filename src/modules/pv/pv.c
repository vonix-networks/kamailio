...
static int w_xavp_copy(
		sip_msg_t *msg, char *src_name, char *src_idx, char *dst_name);
...
static int w_xavp_slist_explode(
		sip_msg_t *msg, char *pslist, char *psep, char *pmode, char *pxname);
static int w_xavp_child_seti(
 ...
 *
 */
static int ki_xavp_copy_dst_mode(
		str *src_name, int src_idx, str *dst_name, int dst_idx, int dimode)
{
	...
	if(dimode) {
		...
		if(dst_idx == 0) {
			...
			if(!prev_xavp) {
				LM_ERR("error inserting xavp, parent not found "
					   "$xavp(%.*s[%d])\n",
 ...
 *
 */
static int ki_xavp_copy(
		sip_msg_t *msg, str *src_name, int src_idx, str *dst_name)
{
 ...
 *
 */
static int ki_xavp_copy_dst(
		sip_msg_t *msg, str *src_name, int src_idx, str *dst_name, int dst_idx)
{
 ...
 */
static int w_xavp_copy(
		sip_msg_t *msg, char *_src_name, char *_src_idx, char *_dst_name)
{
		...
		char *_dst_name, char *_dst_idx)
{
	...
	return ki_xavp_copy_dst_mode(
			&src_name, src_idx, &dst_name, dst_idx, dimode);
}
 ...
 *
 */
static int ki_xavp_slist_explode(
		sip_msg_t *msg, str *slist, str *sep, str *mode, str *sxname)
{
 ...
 *
 */
static int ki_xav_child_seti(
		sip_msg_t *msg, str *rname, str *cname, int ival, int _case)
{
	int ret;
	if(_case) {
...
static int ki_xavp_child_seti(sip_msg_t *msg, str *rname, str *cname, int ival)
{
	return ki_xav_child_seti(msg, rname, cname, ival, 0);
}

static int ki_xavi_child_seti(sip_msg_t *msg, str *rname, str *cname, int ival)
{
 ...
 *
 */
static int w_xav_child_seti(
		sip_msg_t *msg, char *prname, char *pcname, char *pval, int _case)
{
	str rname = STR_NULL;
	str cname = STR_NULL;
	int ival = 0;
 ...
 *
 */
static int ki_xav_child_sets(
		sip_msg_t *msg, str *rname, str *cname, str *sval, int _case)
{
	int ret;
	if(_case) {
...
static int ki_xavp_child_sets(sip_msg_t *msg, str *rname, str *cname, str *sval)
{
	return ki_xav_child_sets(msg, rname, cname, sval, 0);
}

static int ki_xavi_child_sets(sip_msg_t *msg, str *rname, str *cname, str *sval)
{
...
static sr_kemi_xval_t *ki_xavp_get_xval(sr_xavp_t *xavp, int rmode)
{
	...
	switch(xavp->val.type) {
		...
		case SR_XTYPE_STR:
			...
			break;
		case SR_XTYPE_TIME:
			if(snprintf(_pv_ki_xavp_buf, 128, "%lu",
					   (long unsigned)xavp->val.v.t)
					< 0) {
				sr_kemi_xval_null(&_sr_kemi_pv_xval, rmode);
				return &_sr_kemi_pv_xval;
			}
		...
		case SR_XTYPE_LLONG:
			...
			break;
		case SR_XTYPE_XAVP:
			if(snprintf(_pv_ki_xavp_buf, 128, "<<xavp:%p>>", xavp->val.v.xavp)
					< 0) {
				sr_kemi_xval_null(&_sr_kemi_pv_xval, rmode);
				return &_sr_kemi_pv_xval;
			}
			break;
		case SR_XTYPE_DATA:
			if(snprintf(_pv_ki_xavp_buf, 128, "<<data:%p>>", xavp->val.v.data)
					< 0) {
				sr_kemi_xval_null(&_sr_kemi_pv_xval, rmode);
				return &_sr_kemi_pv_xval;
			}
 ...
 *
 */
static sr_kemi_xval_t *ki_xav_get_mode(
		sip_msg_t *msg, str *rname, int rmode, int _case)
{
	sr_xavp_t *xavp = NULL;
	...
	while(avp != NULL) {
		switch(avp->val.type) {
			case SR_XTYPE_XAVP:
				break;
			default:
				val = (sr_kemi_dict_item_t *)pkg_malloc(
						sizeof(sr_kemi_dict_item_t));
				if(val == NULL) {
					PKG_MEM_ERROR;
					goto error;
				}
 ...
 *
 */
static sr_kemi_xval_t *ki_xav_getd_helper(
		sip_msg_t *msg, str *rname, int *_indx, int _case)
{
	sr_xavp_t *xavp = NULL;
	int xavp_size = 0;
	int indx = 0;
 ...
 *
 */
static sr_kemi_xval_t *ki_xav_get_keys(
		sip_msg_t *msg, str *rname, int indx, int _case)
{
	sr_xavp_t *xavp = NULL;
 ...
 *
 */
static int ki_xav_child_is_null(
		sip_msg_t *msg, str *rname, str *cname, int _case)
{
	sr_xavp_t *xavp = NULL;
	if(_case) {
		xavp = xavi_get_by_index(rname, 0, NULL);
 ...
 *
 */
static sr_kemi_xval_t *ki_xav_child_get_mode(
		sip_msg_t *msg, str *rname, str *cname, int rmode, int _case)
{
	sr_xavp_t *xavp = NULL;
 ...
 *
 */
static sr_kemi_xval_t *ki_xavp_child_gete(
		sip_msg_t *msg, str *rname, str *cname)
{
 ...
 *
 */
static sr_kemi_xval_t *ki_xavi_child_gete(
		sip_msg_t *msg, str *rname, str *cname)
{
 ...
 *
 */
static sr_kemi_xval_t *ki_xavp_child_getw(
		sip_msg_t *msg, str *rname, str *cname)
{
 ...
 *
 */
static sr_kemi_xval_t *ki_xavi_child_getw(
		sip_msg_t *msg, str *rname, str *cname)
{
 ...
 *
 */
static int ki_xavu_child_seti(sip_msg_t *msg, str *rname, str *cname, int ival)
{
	sr_xavp_t *xavu = NULL;
 ...
 *
 */
static int ki_xavu_child_sets(sip_msg_t *msg, str *rname, str *cname, str *sval)
{
	sr_xavp_t *xavu = NULL;
 ...
 *
 */
static sr_kemi_xval_t *ki_xavu_child_get_mode(
		sip_msg_t *msg, str *rname, str *cname, int rmode)
{
	sr_xavp_t *xavu = NULL;
 ...
 *
 */
static sr_kemi_xval_t *ki_xavu_child_gete(
		sip_msg_t *msg, str *rname, str *cname)
{
 ...
 *
 */
static sr_kemi_xval_t *ki_xavu_child_getw(
		sip_msg_t *msg, str *rname, str *cname)
{
...
static const char *rpc_shv_set_doc[2] = {
		"Set a shared variable (args: name type value)", 0};

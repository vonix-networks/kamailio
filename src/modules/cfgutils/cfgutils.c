...
static cmd_export_t cmds[] = {
		...
		{"xroutes", (cmd_function)w_routes, 1, fixup_spve_null, fixup_free_spve_null, ANY_ROUTE},
...
{
	if(rpc->rpl_printf(ctx, "actual probability: %u percent", (*probability))
			< 0) {
		rpc->fault(ctx, 500, "Failure building the response");
		return;
	}
...
{
	if(rpc->rpl_printf(ctx, "%.*s", MD5_LEN, config_hash) < 0) {
		rpc->fault(ctx, 500, "Failure building the response");
		return;
	}
}
...
static const char *cfgutils_rpc_is_gflag_doc[2] = {
		"Checks if the bits specified by the argument are all set.", 0};


static const char *cfgutils_rpc_set_gflag_doc[2] = {
		"Sets the bits specified by the argument.", 0};


static const char *cfgutils_rpc_get_gflags_doc[2] = {
		"Return the value for global flags.", 0};


static const char *cfgutils_rpc_reset_gflag_doc[2] = {
		"Resets the bits specified by the argument.", 0};

static const char *cfgutils_rpc_set_prob_doc[2] = {
		"Set the random probability.", 0};
static const char *cfgutils_rpc_reset_prob_doc[2] = {
		"Reset the random probability.", 0};

static const char *cfgutils_rpc_get_prob_doc[2] = {
		"Get the random probability.", 0};

static const char *cfgutils_rpc_get_hash_doc[2] = {"Get config hash value.", 0};

static const char *cfgutils_rpc_check_hash_doc[2] = {
		"Check config hash value.", 0};

static rpc_export_t rpc_cmds[] = {{"cfgutils.is_gflag", cfgutils_rpc_is_gflag,

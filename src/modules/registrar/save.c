		...
		if(ci.received.len == 0) {
			if(_c->received) {
				...
				if(received_found == 0) {
					...
					if(rcv_avp_name.n != 0
							&& search_first_avp(
									rcv_avp_type, rcv_avp_name, &val, 0)
							&& val.s.len > 0) {
		...
		int _use_regid, int novariation)
{
	...
	for(num = 0, r = 0, ci = 0; _c; _c = get_next_contact(_c)) {
		...
		if((ci = pack_ci((ci == 0) ? _m : 0, _c, expires, flags, _use_regid))
				== 0) {
 ...
 */
static inline int add_contacts(struct sip_msg *_m, udomain_t *_d, str *_a,
		int _mode, int _use_regid, int novariation)
{
	int res;
	int ret;
	urecord_t *r;
...
int save(struct sip_msg *_m, udomain_t *_d, int _cflags, str *_uri)
{
	...
	if(reg_outbound_mode != REG_OUTBOUND_NONE && _m->contact
			&& _m->contact->parsed
		...
		if((use_ob == 0) && (reg_regid_mode == REG_REGID_OUTBOUND)) {
			if((get_supported(_m) & F_OPTION_TAG_OUTBOUND) && contact->reg_id) {
	...
	if(c == 0) {
		if(st) {
			if(star(_m, (udomain_t *)_d, &aor, &u->host) < 0)
				goto error;
			else
				ret = 3;
		} else {
			if(no_contacts(_m, (udomain_t *)_d, &aor, &u->host) < 0)
				goto error;
			else
				ret = 4;
		}
		...
		if((ret = add_contacts(
					_m, (udomain_t *)_d, &aor, mode, use_regid, novariation))
				< 0)
			goto error;
		ret = (ret == 0) ? 1 : ret;
	}

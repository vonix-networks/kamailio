...
#ifndef SRC_MODULES_KAZOO3_INCLUDE_UUID_HPP_
#define SRC_MODULES_KAZOO3_INCLUDE_UUID_HPP_
...
//      }
      ...
      {
    	  return (data[8] & 0x80) == 0x00 ? uuid_variant::ncs : (data[8] & 0xC0) == 0x80 ? uuid_variant::rfc : (data[8] & 0xE0) == 0xC0 ? uuid_variant::microsoft : uuid_variant::reserved;
      }

      constexpr uuid_version version() const noexcept
      {
         return (data[6] & 0xF0) == 0x10 ? uuid_version::time_based :
         (data[6] & 0xF0) == 0x20 ? uuid_version::dce_security :
         (data[6] & 0xF0) == 0x30 ? uuid_version::name_based_md5 :
         (data[6] & 0xF0) == 0x40 ? uuid_version::random_number_based :
         (data[6] & 0xF0) == 0x50 ? uuid_version::name_based_sha1 : uuid_version::none;
      }
...
/*
      ...
      {
    	 return std::any_of(data.begin(), data.end(), [](int i){return i != 0;}) ? false : true;
...
//      }
      ...
      static uuid from_string(TChar const * const str, size_t const size)
      {
         ...
         return uuid{ std::begin(data), std::end(data) };
      }
...
#endif /* SRC_MODULES_KAZOO3_INCLUDE_UUID_HPP_ */

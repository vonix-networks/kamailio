 ...
 *  Class definition
 */
class LibEvHandler : public AMQP::TcpHandler
{
private:
    /**
    ...
    private:
        ...
        static void callback(struct ev_loop *loop, struct ev_timer *timer, int revents)
        {
            ...
            AMQP_VERBOSE << "ON EXPIRE " << kz_gettid() << " on " << my_pid();
        ...
        virtual void onExpired() override
        {
            ...
            if (_interval == 0)
            {
                ...
                AMQP_DBG << "close connection timeout " << kz_gettid() << " on " << my_pid();
            ...
            else
            {
                ...
                if ((now - _expire) > 2)
                {
                	...
                	AMQP_DBG << "connection heartbeat expired "
                			<< std::setprecision (std::numeric_limits<double>::digits10 + 1)
                	<< now << " - " << _expire << " : "  << kz_gettid() << " on " << my_pid();
                    _connection->close(true);
                }
                else
                {
                    // find the earliest thing that expires
                    _timer.repeat = _next; //std::min(_next, _expire) - now;
...
//            _next = now + std::max(1, _interval / 2);
            _next = std::max(1, _interval / 2);
            _expire = now + _interval;
...
//            _timer.repeat = std::min(_next, _expire) - now;
        ...
        void beat()
        {
        	AMQP_VERBOSE << "server beat";
        	_expire = ev_now(_loop) + _interval;
        }
        ...
        void monitor(int fd, int events)
        {
            ...
            else
            {
                ...
                {
                    // do we have a match?
                    if (watcher.contains(fd)) {
                    	AMQP_VERBOSE << "found fd in watchers";
     ...
     *  @return Wrapper
     */
    Wrapper &lookup(AMQP::TcpConnection *connection)
    {
        // look for the appropriate connection
        for (auto &wrapper : _wrappers)
        {
            // do we have a match?
            if (wrapper.contains(connection)) {
            	AMQP_VERBOSE << "found wrapper";
            	return wrapper;
            }
        }
        
        // add to the wrappers
        _wrappers.emplace_back(_loop, connection, _connect_timeout);
     ...
     */
    virtual void monitor(AMQP::TcpConnection *connection, int fd, int flags) override final
    {
    	AMQP_VERBOSE << "MONITOR FD " << fd << " flags:" << flags;
...
public:
     ...
     */
    LibEvHandler(struct ev_loop *loop, uint16_t connect_timeout = 60) :
    	_loop(loop),

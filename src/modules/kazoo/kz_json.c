...
enum json_type kz_json_get_type(json_t * jso)
{
...
json_t * kz_json_get_str_field_object(json_t * json, str* field)
{
	json_keys keys;
	json_key key;
	char* dup;
...
json_t * kz_json_get_field_object(json_t * json, const char * field)
{
	str sfield;
	sfield.s = (char *) field;
	sfield.len = strlen(field);
	return kz_json_get_str_field_object(json, &sfield);
}

json_t * kz_json_parse_get_field_object(str* str_json, str* field)
{
	char* dup = NULL;
	json_t *jtree = NULL;
	json_t *ret = NULL;
	...
	dup = pkg_malloc(str_json->len+1);
	memcpy(dup, str_json->s, str_json->len);
	dup[str_json->len] = '\0';
	json = json_tokener_parse(dup);
	pkg_free(dup);
	...
	if((jtree = kz_json_get_str_field_object(json, field)) != NULL) {
...
int kz_json_get_count(str* json, str* field, pv_value_p dst_val)
{
	...
	json_t *jtree = kz_json_parse_get_field_object(json, field);
...
json_t * kz_json_parse(const char *str)
{
    struct json_tokener* tok;
    struct json_object* obj;

    if (!str) {
      LM_ERR("bad arg parsing json: input is null\n");
      return NULL;
    }
...
json_t * kz_json_get_object(json_t * jso, const char *key)
{
...
int kz_json_get_keys(struct sip_msg* msg, char* json, char* field, char* dst)
{
  ...
  pv_spec_t *avp_spec;
	...
	json_t *jtree = kz_json_parse_get_field_object(&json_s, &field_s);

...
private:
    ...
    void process(BasicCancelFrame &frame);
    ...
    virtual const std::shared_ptr<Deferred> &reportCanceled(const std::string &name);
...
public:
    ...
    DeferredConsumer &onConsuming(const ConsumeCallback &callback)
    {
        // store the callback
        _consumeCallback = callback;
     ...
     *  @param  callback
     */
    DeferredConsumer &onCanceled(const CanceledCallback &callback)
    {
        // store the callback
     ...
     *  @param  callback
     */
    DeferredConsumer &onError(const ErrorCallback &callback)
    {
        // store the callback

...
bool ChannelImpl::reportCanceled(const std::string& consumerTag)
{
    BasicCancelOKFrame frame(_id, consumerTag);
    ...
    if(iter != _consumers.end()) {
    	iter->second->reportCanceled(consumerTag);
    }

    // remove consumer
    uninstall(consumerTag);

 ...
 *  @return Deferred
 */
const std::shared_ptr<Deferred> &DeferredConsumer::reportCanceled(const std::string &name)
{
    // skip if no special callback was installed
    if (_canceledCallback) {
    	_canceledCallback(name);
    }

...
int kz_tr_init_buffers(void)
{
	...
	_kz_tr_buffer_list = (char**)pkg_malloc(KZ_TR_BUFFER_SLOTS * sizeof(char*));

	if(_kz_tr_buffer_list==NULL)
		return -1;
	for(i=0; i<KZ_TR_BUFFER_SLOTS; i++) {
		_kz_tr_buffer_list[i] = (char*)pkg_malloc(KZ_TR_BUFFER_SIZE);
		if(_kz_tr_buffer_list[i]==NULL)
			return -1;
	}

	_kz_parse_specs = (pv_spec_t**)pkg_malloc(KZ_TR_ALLOC_PARSE_SIZE * sizeof(pv_spec_t*));
	for(i=0; i < KZ_TR_ALLOC_PARSE_SIZE; i++)
		_kz_parse_specs[i] = NULL;

	_kz_parse_params = (tr_param_t**)pkg_malloc(KZ_TR_ALLOC_PARSE_SIZE * sizeof(tr_param_t*));
	for(i=0; i < KZ_TR_ALLOC_PARSE_SIZE; i++)
...
void kz_tr_clear_buffers(void)
{
	...
	if(_kz_tr_buffer_list != NULL) {
		for(i=0; i<KZ_TR_BUFFER_SLOTS; i++) {
			if(_kz_tr_buffer_list[i] != NULL) {
				pkg_free(_kz_tr_buffer_list[i]);
				_kz_tr_buffer_list[i] = NULL;
			}
		}
		pkg_free(_kz_tr_buffer_list);
	...
	if(_kz_parse_specs != NULL) {
		for(i=0; i<KZ_TR_ALLOC_PARSE_SIZE; i++) {
			if(_kz_parse_specs[i] != NULL) {
				pkg_free(_kz_parse_specs[i]);
				_kz_parse_specs[i] = NULL;
			}
		}
		pkg_free(_kz_parse_specs);
	...
	if(_kz_parse_params != NULL) {
		for(i=0; i<KZ_TR_ALLOC_PARSE_SIZE; i++) {
			if(_kz_parse_params[i] != NULL) {
				pkg_free(_kz_parse_params[i]);
				_kz_parse_params[i] = NULL;
			}
		}
		pkg_free(_kz_parse_params);
...
int kz_tr_eval(struct sip_msg *msg, tr_param_t *tp, int subtype, pv_value_t *val)
{
	...
	switch(subtype)
	{
		...
		case TR_KAZOO_JSON:
			...
			if(pv == NULL)
			{
				LM_ERR("kazoo json transform : no more private memory\n");
				return -1;
			}
...
#define _kz_tr_parse_sparam(_p, _p0, _tp, _spec, _ps, _in, _s) \
	...
	{ /* pseudo-variable */ \
		...
		_tp->v.s.len = _p - _ps; \
		_tp->v.s.s = (char*)pkg_malloc((tp->v.s.len+1)*sizeof(char)); \
...
char* kz_tr_parse(str* in, trans_t *t)
{
	...
	if(name.len==6 && strncasecmp(name.s, "encode", 6)==0)
	{
		...
		goto done;
	} else if(name.len==8 && strncasecmp(name.s, "filename", 8)==0) {
		t->subtype = TR_KAZOO_FILENAME;
		goto done;
	}

	LM_ERR("unknown kazoo transformation: %.*s/%.*s/%d!\n", in->len, in->s, name.len, name.s, name.len);
error:
	if(tp)
		pkg_free(tp);
	if(spec)

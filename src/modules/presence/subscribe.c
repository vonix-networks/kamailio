		...
		if(delete_shtable(subs_htable, hash_code, &subs) < 0) {
			LM_ERR("Failed to delete subscription from memory"
				   " [slot: %u ev: %.*s pu: %.*s ci: %.*s ft: %.*s tt: %.*s]\n",
					hash_code, pres_uri->len, pres_uri->s, ev_name->len,
					ev_name->s, callid->len, callid->s, from_tag->len,
					from_tag->s, to_tag->len, to_tag->s);
		}
	}
...
int update_subscription_notifier(
		struct sip_msg *msg, subs_t *subs, int to_tag_gen, int *sent_reply)
{
	...
	*sent_reply = 0;
	...
	if(subs->event->type & WINFO_TYPE)
		...
		if((num_peers = set_wipeer_subs_updated(
					&subs->pres_uri, subs->event->wipeer, subs->expires == 0))
				< 0) {
	...
	*sent_reply = 0;
	...
	if(to_tag_gen == 0) /*if a SUBSCRIBE within a dialog */
	{
		if(subs->expires == 0) {
			...
			if(subs->event->type & PUBL_TYPE) {
				...
				*sent_reply = 1;
				if(subs->event->wipeer) {
					if(query_db_notify(
							   &subs->pres_uri, subs->event->wipeer, NULL)
							< 0) {
			...
			{
				if(send_2XX_reply(msg, 200, subs->expires, &subs->local_contact)
						< 0) {
	...
	if(subs->event->type & PUBL_TYPE) {
		...
		*sent_reply = 1;
		...
		if(send_2XX_reply(msg, 200, subs->expires, &subs->local_contact) < 0) {
			LM_ERR("sending 200 OK reply\n");
			goto error;
		}
		*sent_reply = 1;
...
int pv_get_subscription(struct sip_msg *msg, pv_param_t *param, pv_value_t *res)
{
	...
	if(param->pvn.u.isname.name.n == 1) {
		...
		return pv_get_strval(
				msg, param, res, &_pres_subs_last_sub->from_domain);
	} else if(param->pvn.u.isname.name.n == 6) {
		return pv_get_strval(
				msg, param, res, &_pres_subs_last_sub->watcher_user);
	} else if(param->pvn.u.isname.name.n == 7) {
		return pv_get_strval(
				msg, param, res, &_pres_subs_last_sub->watcher_domain);
	} else if(param->pvn.u.isname.name.n == 8) {
		return pv_get_strval(
				msg, param, res, &_pres_subs_last_sub->event->name);
		...
		return pv_get_uintval(
				msg, param, res, _pres_subs_last_sub->remote_cseq);
		...
		return pv_get_strval(
				msg, param, res, &_pres_subs_last_sub->local_contact);
	} else if(param->pvn.u.isname.name.n == 17) {
		return pv_get_strval(
				msg, param, res, &_pres_subs_last_sub->record_route);
...
int handle_subscribe0(struct sip_msg *msg)
{
	...
	return handle_subscribe(
			msg, pfrom->parsed_uri.user, pfrom->parsed_uri.host);
}
	...
	if(to_tag_gen) {
		...
		if(!event->req_auth) {
			...
			if(found == 0) {
				...
				if(subs.event->get_rules_doc(
						   &uri.user, &uri.host, &subs.auth_rules_doc)
						< 0) {
...
int extract_sdialog_info_ex(subs_t *subs, struct sip_msg *msg, uint32_t miexp,
		uint32_t mexp, int *to_tag_gen, str scontact, str watcher_user,
		str watcher_domain, int *reply_code, str *reply_str)
{
	str rec_route = {0, 0};
	int rt = 0;
	contact_body_t *b;
	...
	if(parse_headers(msg, HDR_USERAGENT_F, 0) != -1 && msg->user_agent
			...
			&& msg->user_agent->body.len < MAX_UA_SIZE) {
		...
		struct sip_msg *msg, subs_t *subs, int *reply_code, str *reply_str)
{
	...
	s = search_shtable(
			subs_htable, subs->callid, subs->to_tag, subs->from_tag, hash_code);
	if(s)
		goto found_rec;
	...
	for(i = 0; i < shtable_size; i++) {
		...
		s = search_shtable(
				subs_htable, subs->callid, subs->to_tag, subs->from_tag, i);
		if(s) {
		...
		struct sip_msg *msg, subs_t *subs, int *reply_code, str *reply_str)
{
	...
	if(record_route.s) {
		...
		if(record_route.len > 0) {
			subs->record_route.s =
					(char *)pkg_malloc(record_route.len * sizeof(char));
			if(subs->record_route.s == NULL) {
				ERR_MEM(PKG_MEM_STR);
			}
...
void update_db_subs_timer_notifier(void)
{
	...
	if(pa_dbf.query_lock)
		res = db_fetch_query_lock(&pa_dbf, pres_fetch_rows, pa_db, query_cols,
				query_ops, query_vals, result_cols, n_query_cols, n_result_cols,
				0, &result);
	else
...
void update_db_subs_timer_dbonly(void)
{
	...
	int pres_uri_col, to_user_col, to_domain_col, from_user_col,
			from_domain_col, callid_col, totag_col, fromtag_col, event_col,
			event_id_col, local_cseq_col, expires_col, rr_col, sockinfo_col,
	...
	res = db_fetch_query(&pa_dbf, pres_fetch_rows, pa_db, qcols, qops, qvals,
			result_cols, 1, n_result_cols, 0, &result);
	if(res < 0) {
		...
		while(s) {
			...
			switch(s->db_flag) {
				...
				case UPDATEDB_FLAG:
					...
					update_vals[u_record_route_col].val.str_val =
							s->record_route;
				...
				case INSERTDB_FLAG:
					...
					query_vals[from_domain_col].val.str_val = s->from_domain;
					query_vals[watcher_user_col].val.str_val = s->watcher_user;
					query_vals[watcher_domain_col].val.str_val =
							s->watcher_domain;
					query_vals[event_col].val.str_val = s->event->name;
					query_vals[event_id_col].val.str_val = s->event_id;
					query_vals[local_cseq_col].val.int_val = s->local_cseq;
...
int restore_db_subs(void)
{
	...
	do {
		...
		for(i = 0; i < nr_rows; i++) {
			...
			s.watcher_user.s =
					(char *)row_vals[watcher_user_col].val.string_val;
			s.watcher_user.len = strlen(s.watcher_user.s);
			...
			s.local_contact.s =
					(char *)row_vals[local_contact_col].val.string_val;
			s.local_contact.len = strlen(s.local_contact.s);

			s.record_route.s =
					(char *)row_vals[record_route_col].val.string_val;
			if(s.record_route.s)
...
static int ps_get_db_subs_auth(subs_t *subs, int *found)
{
	...
	if(pa_dbf.query(pa_db, db_keys, 0, db_vals, result_cols, n_query_cols, 2, 0,
			   &result)
			< 0) {
	...
	*found = 1;
	...
	if(row_vals[1].val.string_val) {
		...
		if(subs->reason.len == 0)
			...
			subs->reason.s =
					(char *)pkg_malloc(subs->reason.len * sizeof(char));
			if(subs->reason.s == NULL) {
				pa_dbf.free_result(pa_db, result);
				ERR_MEM(PKG_MEM_STR);
			}
...
static int ps_insert_db_subs_auth(subs_t *subs)
{
	...
	if(pa_dbf.replace != NULL) {
		if(pa_dbf.replace(pa_db, db_keys, db_vals, n_query_cols, 2, 0) < 0) {

...
int db_check_api(db_func_t *dbf, char *mname)
{
	...
	if(dbf->use_table == 0) {
		LM_ERR("module %s does not export db_use_table function. Please check "
			   "if module is loaded.\n",
				mname);
		goto error;
	}
	...
	if(dbf->init == 0) {
		LM_ERR("module %s does not export db_init function. Please check if "
			   "module is loaded.\n",
				mname);
		goto error;
	}
	...
	if(dbf->close == 0) {
		LM_ERR("module %s does not export db_close function. Please check if "
			   "module is loaded.\n",
				mname);
		goto error;
	}
...
int db_bind_mod(const str *mod, db_func_t *mydbf)
{
	...
	if(dbind != NULL) {
		...
		dbf.use_table =
				(db_use_table_f)find_mod_export(tmp, "db_use_table", 2, 0);
		...
		dbf.fetch_result = (db_fetch_result_f)find_mod_export(
				tmp, "db_fetch_result", 2, 0);
		dbf.raw_query =
				(db_raw_query_f)find_mod_export(tmp, "db_raw_query", 2, 0);
		dbf.free_result =
				(db_free_result_f)find_mod_export(tmp, "db_free_result", 2, 0);
		...
		dbf.last_inserted_id = (db_last_inserted_id_f)find_mod_export(
				tmp, "db_last_inserted_id", 1, 0);
		dbf.affected_rows = (db_affected_rows_f)find_mod_export(
				tmp, "db_affected_rows", 1, 0);
		dbf.insert_update = (db_insert_update_f)find_mod_export(
				tmp, "db_insert_update", 2, 0);
		dbf.insert_delayed = (db_insert_delayed_f)find_mod_export(
				tmp, "db_insert_delayed", 2, 0);
		dbf.start_transaction = (db_start_transaction_f)find_mod_export(
				tmp, "db_start_transaction", 2, 0);
		dbf.end_transaction = (db_end_transaction_f)find_mod_export(
				tmp, "db_end_transaction", 1, 0);
		dbf.abort_transaction = (db_abort_transaction_f)find_mod_export(
				tmp, "db_abort_transaction", 1, 0);
		dbf.query_lock =
				(db_query_f)find_mod_export(tmp, "db_query_lock", 2, 0);
	}
 ...
 */
int db_table_version(
		const db_func_t *dbf, db1_con_t *connection, const str *table)
{
...
int db_load_bulk_data(db_func_t *binding, db1_con_t *handle, str *name,
		db_key_t *cols, unsigned int count, unsigned int strict, db1_res_t *res)
{
	if(binding == NULL) {
